...
/// 選択された接続方式のアダプター候補を読み込むユースケースです。
@MainActor
struct DiscoverAdaptersUseCase {
    ...
    /// 指定された接続方式で現在参照できる候補を取得します。
    ///
    /// 責務: 1件の接続方式指定をアダプター探索境界へ委譲します。
    ...
    func execute(for mode: AdapterTransportMode) async throws -> [DiscoveredAdapter] {
        ...
        let uniqueAdapters = Dictionary(adapters.map { ($0.id, $0) }, uniquingKeysWith: { first, _ in first })
