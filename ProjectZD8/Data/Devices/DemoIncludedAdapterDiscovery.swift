/// 実デバイス探索へ指定されたデモ候補を追加します。
@MainActor
struct DemoIncludedAdapterDiscovery: AdapterDiscoveryPort {
    /// OS由来候補を返す探索境界です。
    private let wrapped: any AdapterDiscoveryPort
    /// 実探索結果へ追加するデモ候補です。
    private let demoCandidate: DiscoveredAdapter

    /// 実探索境界と追加対象デモ候補を保持して生成します。
    ///
    /// 責務: 1件の実探索境界を指定接続方式のデモ候補追加処理へ固定します。
    /// - Parameters:
    ///   - wrapped: OS由来候補を取得する探索境界。
    ///   - demoCandidate: 一致する接続方式へ常時追加するデモ候補。
    init(wrapping wrapped: any AdapterDiscoveryPort, demoCandidate: DiscoveredAdapter) {
